...
def test_super_user_email_is_normalized(super_user):
    ...
    eamil = "alpha@REALESTATE.COM"
    assert super_user.email == eamil.lower()
...
def test_create_superuser_with_no_password(user_factory):
    """Test creating a superuser without an password raises an error"""
    with pytest.raises(ValueError) as err:
        user_factory.create(is_superuser=True, is_staff=True, password=None)
    assert str(err.value) == "Superusers must have password"

def test_user_email_incorrect(user_factory):
    """Test that an error is raised when a Invalid email is provided"""
    with pytest.raises(ValueError) as err:
